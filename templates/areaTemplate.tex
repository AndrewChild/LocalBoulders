%% macro image(filePath, description=None, size='h', im_scale=1.0)
	%% if size == 'f'
		\end{multicols}
		\begin{center}
			\centering
				\includegraphics[width=\VAR{im_scale}\textwidth]{\VAR{filePath}}
		\end{center}
		\raggedcolumns
		\begin{multicols}{2}
	%% else
		\setbox0=\hbox{\includegraphics[width=\VAR{im_scale}\linewidth]{\VAR{filePath}}}% Store image in \box0
		\needspace{\ht0}% Need at least the height of \box0
		\begin{center}
		\includegraphics[width=\VAR{im_scale}\linewidth]{\VAR{filePath}}
		\end{center}
	%% endif
%% endmacro
%% macro climbLabel(climb)
\label{\VAR{climb.ref}:\VAR{climb.name}}
\colorbox{\VAR{climb.color_LaTeX}}{
\parbox{0.95\textwidth}{
\textbf{
\VAR{climb.getRtNum()} \VAR{climb.name}\VAR{climb.name_unconfirmed_LaTeX} V\VAR{climb.grade}\VAR{climb.grade_unconfirmed_LaTeX} \VAR{climb.rating_LaTeX} \VAR{climb.serious_LaTeX}
}
}
}
%% endmacro
\formatChapter{\VAR{area.name}}
\raggedcolumns
\begin{multicols}{2}
%% for map in area.areaMaps
	\VAR{image(map.path_o+map.fileName, map.description, map.size, map.im_scale)}
%% endfor
%% if area.gps
	\qrcode{\VAR{area.paths['qr_o']}/\VAR{area.name}_qr.png}{http://maps.google.com/maps?q=\VAR{area.gps}}{Navigate to this area}
%% endif
\begin{minipage}{\columnwidth}
\VAR{area.description or "\\"}
%% if area.incomplete
\newline
\textbf{NOTE: This area is mostly incomplete. Look forward to more information in future revisions of this book or contribute your own knowledge on github.}
%% endif
\end{minipage}
\halfPic{\VAR{area.paths['histogram_o']}/\VAR{area.name}.png}

%% for subArea in area.subareas.values()
\newpage
	%% for map in subArea.subAreaMaps
		\VAR{image(map.path_o+map.outFileName, map.description, map.size, map.im_scale)}
	%% endfor
	%% if subArea.name
		\section{\VAR{subArea.name}}\label{sa:\VAR{subArea.name}}
	%% endif
	%% if subArea.gps
		\qrcode{\VAR{area.paths['qr_o']}\VAR{subArea.name}_qr.png}{http://maps.google.com/maps?q=\VAR{subArea.gps}}{Navigate to this sub area}
	%% endif
	\begin{minipage}{\columnwidth}
	\VAR{subArea.description or "\\"}
	\end{minipage}
	
	%% for photo in subArea.photos
		%# \VAR{image(photo.path+photo.fileName, photo.description, photo.size, photo.im_scale)}\label{pt:\VAR{photo.name}}
	%% endfor
	
	%% if subArea.boulders
		%% for boulder in subArea.boulders.values()
			%% for topo in boulder.topos
				%% if topo.size == 'f'
				\VAR{image(topo.path_o+topo.outFileName, topo.description, topo.size, topo.im_scale)}
				%% endif
			%% endfor
			\begin{minipage}{\columnwidth}
			\subsection*{\VAR{boulder.name}}\label{bf:\VAR{boulder.name}}
			\VAR{boulder.description or "\\"}
			
			%% for photo in boulder.photos
				%# \VAR{image(photo.path+photo.fileName, photo.description, photo.im_scale, photo.scale)}\label{pt:\VAR{photo.name}}
			%% endfor
			\end{minipage}
			
			%% if boulder.routes
				%% for route in boulder.routes.values()
					%% for topo in route.topos
						%% if topo.size == 'h'
						\VAR{image(topo.path_o+topo.outFileName, topo.description, topo.size, topo.im_scale)}
						%% endif
					%% endfor
					\begin{minipage}{\linewidth}	
					\VAR{climbLabel(route)}
					\begin{adjustwidth}{0.5cm}{}				
					\VAR{route.description}
					%% if not route.hasTopo
						\newline (No Topo) 
					%% endif
					\end{adjustwidth}
					\end{minipage}
					%% if route.variations.values()
						\begin{adjustwidth}{0.5cm}{}				
						\textbf{Variations:} \newline
						%% for variation in route.variations.values()
							%% for topo in variation.topos
								\VAR{image(topo.path_o+topo.outFileName, topo.description, topo.size, topo.im_scale)}
							%% endfor
							\begin{minipage}{\linewidth}	
							\VAR{climbLabel(variation)}
							\begin{adjustwidth}{0.5cm}{}				
							\VAR{variation.description}
							%% if not variation.hasTopo
								\newline (No Topo) 
							%% endif
							\end{adjustwidth}
							\end{minipage}
						%% endfor
						\end{adjustwidth}
					%% endif
				%% endfor 
			%% endif
			
		%% endfor 
	%% endif
%% endfor


\end{multicols}
\clearpage