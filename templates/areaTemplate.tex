...
%% if map.loc == loc
%% if map.size == size
\VAR{image(map.path_o+map.outFileName, map.description, 'h')}
%% endif
%% endif
%% endfor
%% for photo in item.photos
%% if photo.loc == loc
%% if photo.size == size
\VAR{image(photo.path+photo.fileName, photo.description, 'h')}\label{pt:\VAR{photo.name}}
%% endif
%% endif
%% endfor
%% endmacro
...
\end{multicols}
%% endif
\setbox0=\hbox{\begin{overpic}[width=0.8\linewidth]{\VAR{filePath}}\VAR{caption(description)}
\end{overpic}}
\needspace{\ht0}
\begin{center}
\begin{overpic}[width=0.9\linewidth]{\VAR{filePath}}\VAR{caption(description)}
\end{overpic}
\end{center}
%% if size == 'f'
\raggedcolumns
